...
- [x] Configure error tracking (Sentry or equivalent) — `sentry/sentry-laravel` installed and wired into `bootstrap/app.php`'s exception handler, `SENTRY_LARAVEL_DSN`/`SENTRY_ENVIRONMENT` added to `.env`/`.env.example`. **Inactive** (Sentry's SDK no-ops entirely without a DSN) until the user provides a real Sentry project DSN — this is an external-service credential this session can't generate, matching the same "prepare the integration, wait for real credentials" pattern used for AI provider API keys.
- [x] **Found and fixed during this pass (not originally a checklist item, but a real production security gap)**: the live server's `.env` had `APP_ENV=local` and `APP_DEBUG=true` — meaning any unhandled exception on the real, public site would have rendered Laravel's full debug/stack-trace page (file paths, query bindings, potentially config values) to any visitor. Flipped to `APP_ENV=production`/`APP_DEBUG=false` and verified the site (including a real 404) still serves correctly.
